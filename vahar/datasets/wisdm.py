...
class WisdmParquet(ParquetDatasetFormatter):
    def __init__(self, raw_folder: str, destination_folder: str, sampling_rates: dict,
        ...
        Args:
            ...
            max_interval: dict[submodal] = maximum intervals (millisecond) between rows of an uninterrupted segment;
                default = 500 ms
        """
        max_interval = {WisdmConst.MODAL_ACC: 500} if max_interval is None else max_interval
    ...
    def read_csv_raw(self, filename: str = 'WISDM_ar_v1.1_raw.txt'):
        ...
        df = df.drop_nulls()
        df = df.cast({
            'subject': pl.Int32, 'label': pl.String, 'timestamp(ms)': pl.Int64,
            'pocket_acc_x(m/s^2)': pl.Float64, 'pocket_acc_y(m/s^2)': pl.Float64, 'pocket_acc_z(m/s^2)': pl.Float64
        })
        df = df.filter(pl.col('timestamp(ms)') != 0)

        # convert timestamp unit and data unit
        df = df.with_columns(
            pl.col('timestamp(ms)') * 1e-6,
    ...
    def split_segments(self, df: pl.DataFrame, drop_col: tuple = ('subject',)) -> List[pl.DataFrame]:
        ...
        results = [
            segment_df[WisdmConst.MODAL_ACC].join_asof(label_df, on='timestamp(ms)', strategy='nearest')
    ...
    def run(self):
        ...
        for subject_id, subject_df in raw_df.group_by(['subject']):
            ...
            if os.path.isfile(self.get_output_file_path(WisdmConst.MODAL_ACC, subject_id, 'last')):
            ...
            for seg_i, seg_df in enumerate(segment_dfs):
                ...
                self.write_output_parquet(seg_df, WisdmConst.MODAL_ACC, subject_id, session_id)
                written_files += 1
...
if __name__ == '__main__':
    ...
    #     sampling_rates={WisdmConst.MODAL_ACC: 50}
    # ).run()
